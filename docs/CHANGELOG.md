...
### Added
- **`/flask` endpoint**: New JSON endpoint proving Flask/Python stack is working
  - Returns: `flask` version, `python_version`, `timestamp`
- **Professional status dashboard**: Light theme HTML page at `/` with:
  - Service status cards (Online, Python 3.13, Flask 3.1.3)
  - Available endpoints table with method badges
  - Responsive design, no external dependencies

### Fixed
- **Makefile**: Container name `mv-python-container` corrected to `pypong-python-container`
...
### Changed
- **Architecture**: Updated from PHP stack to Python/Flask stack
- **Documentation**: Complete rewrite of `ARCHITECTURE.md` and `USER_MANUAL.md`
- **New docs**: `API.md` added with Python API reference
